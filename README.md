...
```mermaid
flowchart TD
    subgraph Phase_1_Input ["Phase 1: Input & Excavation"]
        direction TB
        TX["Text Logs (story.txt)"]
        IM["Images (.jpg/.png)"]
        AU["Audio (.mp3)"]
    end

    subgraph Phase_2_Engine ["Phase 2: ChimeraCore Engine"]
        direction TB
    ...
    subgraph Phase_3_Storage ["Phase 3: Chimera Stream"]
        direction TB
        CDS[("story_pac.cds")]
    end

    subgraph Phase_4_Output ["Phase 4: Synthesis"]
        direction TB
