...
graph TD
    ...
    TX[Text Data Source<br/>(story.txt / Chat Logs)]
    IM[Image Data Source<br/>(.jpg, .png)]
    AU[Audio Data Source<br/>(.mp3)]
    end

    subgraph "Phase 2: ChimeraCore Engine (Python)"
    Scrubber[PII Scrubber &<br/>Anonymizer]
    Parser[Log Parser &<br/>Normalizer]
    Analyzer[Sentiment &<br/>Context Analyzer]
    end
    ...
    LLM[Large Language Model<br/>(Context Window)]
